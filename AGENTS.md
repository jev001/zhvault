...
Useful flags: `--data-dir`, `--engine`, `--source`, `--user`, `--full`, `--collection-id`, `--x-zse-96` (full browser token, usually `2.0_…` — not `1`), `--asset-workers`, `--asset-link`, `--json`, `--vector-backend`, `--embed-provider`, `--embed-model`, `--embed-api-base`, `--embed-api-key`, `--from-data-dir`, `--map-collection`, `--offset`, `--i-understand-danger`, `--confirm`.
...
- `--user <url_token|people-URL|/token>` selects a member profile; verified via `GET /members|people/{token}` (members first). `--source people` requires `--user` (docs: placeholders only). With `--user`, ignore `url.json` collections unless `--collection-id` is set; `all` discovers that member’s collections and adds answer/article/column/zvideo/activity. Profile list APIs use a registry (`zhihu_lists.py`): e.g. collections → `/people/.../collections`, columns → `/members/.../column-contributions`, with members/people fallbacks. List 404s soft-skip.
- `--source column` expands each column via `/columns/{key}/items` and fetches article detail (`/articles/{id}` with zhuanlan Referer; on API `10003`/empty falls back to `https://zhuanlan.zhihu.com/p/{id}` HTML). Column MD gets `## Articles` wikilinks.
